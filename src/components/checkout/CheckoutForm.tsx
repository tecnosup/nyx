...
export function CheckoutForm({ product, size }: Props) {
  ...
  async function lookupCep(raw: string) {
    ...
    try {
      ...
      // silêncio: autofill é conveniência, não bloqueia
    } finally {
      setLoadingCep(false);
    }
  }
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        setServerError("Não foi possível validar o pedido. Confira os dados.");
        return;
      }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-10" noValidate>
      ...
      <section className="space-y-5">
        ...
        <Field label="CEP" error={errors.shipping?.cep?.message}>
          <div className="relative">
            ...
            {loadingCep && (
              <Loader2
                size={16}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-nyx-muted animate-spin"
              />
            )}
          </div>
        </Field>
        ...
        <Field
          ...
          error={errors.shipping?.complement?.message}
        >
          <input
            type="text"
      ...
      <p className="text-xs text-nyx-muted leading-relaxed">
        ...
        A Giovanna confirma frete, prazo e forma de pagamento antes da cobrança.
      </p>
    </form>
  );
}
